...
module.exports = async function handler(req, res) {
  ...
  try {
    // POST /api/shorten (or just /shorten via rewrite)
    if (path === '/api/shorten' && method === 'POST') {
      ...
      if (customCode && typeof customCode === 'string') {
        ...
        let tries = 0;
        while (await redis.get(`link:${code}`) && tries < 10) {
          code = generateShortCode();
          tries++;
        }
      }
      ...
      await redis.set(`link:${code}`, linkData);
    ...
    const detailedMatch = path.match(/^\/api\/stats\/detailed\/(.+)$/);
    if (detailedMatch && method === 'GET') {
      const code = detailedMatch[1];
      const link = await redis.get(`link:${code}`);
    ...
    // GET /:code (redirect) - catch non-API, non-static routes
    if (method === 'GET' && !path.startsWith('/api/') && !path.includes('.')) {
      const code = path.slice(1);
      if (!code || code === 'api' || code === 'privacy' || code === 'robots.txt' || code === 'sitemap.xml') {
